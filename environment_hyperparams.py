...
# Main differnece: Low variability in P and Q to the grid. 
P_min = 0.7
P_max = 0.7 
Q_min = 0.0
Q_max = 0.0
class Env_v1(gym.Env):
    ...
    def reset(self):
        ...
        self.P_out_req = np.random.uniform(P_min, P_max)*sum(self.Sn)
        self.Q_out_req = np.random.uniform(Q_min, Q_max)*sum(self.Sn)
